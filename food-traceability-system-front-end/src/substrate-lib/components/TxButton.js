  ...
  const txResHandler = ({ status, events, dispatchError }) => {
    ...
    if (dispatchError) {
      if (dispatchError.isModule) {
        ...
        // const decoded = api.registry.findMetaError(dispatchError);
        const asU8a = dispatchError.asModule.toU8a();
        const decoded = api.registry.findMetaError(asU8a);
      ...
      // Handle SUDO transaction result
      events
        ...
        .forEach(({ event: { data: [result] } }) => {
          if (result.isError) {
            ...
            if (error.isModule) {
              const asU8a = error.asModule.toU8a();
              const decoded = api.registry.findMetaError(asU8a);
              const { docs, name, section } = decoded;

              setStatus(`SUDO Error: ${section}.${name}: ${docs.join(' ')}`);
            } else {
              // Other, CannotLookup, BadOrigin, no extra info
              setStatus(`SUDO Error: ${error.toString()}`);
            }
          }
        });
    }
  ...
  const rpcResHandler = result => {
    if (result.isNone) {
      ...
      if (res === '<Error>') {
        ...
        setStatus('https://api.qrserver.com/v1/create-qr-code/?data=' + res)
      }
      // if (res !== '<Error>') {
      //   // const formData = new FormData();
      //   // formData.append('api_dev_key', 'erMX7uiC77jlHAQQuYOHagYGcES3g_2u');
      //   // formData.append('api_option', 'paste');
      //   // formData.append('api_paste_code', res);

      //   const params = new URLSearchParams();
      //   params.append('api_dev_key', 'erMX7uiC77jlHAQQuYOHagYGcES3g_2u');
      //   params.append('api_option', 'paste');
      //   params.append('api_paste_code', res);
      ...
      //     // body: JSON.stringify({
      //     //   // Add parameters here
      //     //   api_dev_key: "erMX7uiC77jlHAQQuYOHagYGcES3g_2u",
      //     //   api_option: "paste",
      //     //   api_paste_code: res
      //     // }),
      //     // body: formData,
